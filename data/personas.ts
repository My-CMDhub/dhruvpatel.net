...
export const forCase: Record<string, Partial<Record<PersonaId, string>>> = {
  ovela: {
    ...
    engineer: 'The first request goes out while the greeting is still playing, so the cold start hides inside audio the caller is already hearing. The agent asks who’s calling before it looks anything up — twelve of fifteen replayed calls had been spending a lookup to learn nothing. Two clocks run on this page and each figure says which: timed from the moment the speech model calls the turn over, or timed from the caller’s actual last word on the recording. The second is about 0.5–1.0 s longer, and most of that gap is end-of-turn detection and the distance to a US server, not my code.',
    ...
    recruiter: 'Voice AI on a live phone line, built solo. Python, FastAPI, asyncio, Twilio, STT, LLM, TTS, Appwrite. First reply 3.7 s → 0.9 s, one call before and one after. 1,949 tests.',
  },
  'agent-os': {
