...
<CaseHead
  title="Ovela"
  result="A voice receptionist on a real phone line. It answers in 0.55–0.71 s when no tool is needed, and it's built around the parts of a call that usually go wrong."
...
When the model reached for a tool, the caller heard nothing for 2.3–4.3 s. Now code says a short line first ("Let me have a look."), and the first words arrive in 0.7–0.8 s. The answer itself still waits on the tool; the silence is gone.
...
<ul className="sources">
  <Src id="readme" href={`${repo.ovela}#readme`}>Ovela README: the results table (each before/after row is one call each side) and known limits</Src>
</ul>
